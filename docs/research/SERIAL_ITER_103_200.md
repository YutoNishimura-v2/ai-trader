...
**Waves I–P data note:** Waves A–H rows used the project reference M1 CSV
(`data/xauusd_m1_2026.csv`, not tracked). **Waves I–P** used **synthetic** M1
when gold was absent in CI. **Wave Q (179–182)** is recorded on **gold** below
once `data/xauusd_m1_2026.csv` is present (Dukascopy fetch).
...
## Wave O (171–174) — synthetic stand-in harness

| Iter | Config | cap | W | WS | minMA | Mar | Apr | full% | Verdict |
|------|--------|:---:|:--:|-----:|------:|----:|----:|------:|---------|
| 171 | `iter171_rollwin_chop_tp11_moon.yaml` | 0 | 0 | -inf | -10.11 | -10.11 | -7.19 | -4.7 | **NO-OP** (bit-identical vs rollwin on synth) |
| 172 | `iter172_rollwin_trend_tp20.yaml` | 0 | 0 | -inf | -9.94 | -9.94 | -6.04 | 5.3 | **Revalidate** (synth: Mar/Apr & full% vs rollwin; still 0/4) |
| 173 | `iter173_rollwin_chop_max7.yaml` | 0 | 0 | -inf | -10.11 | -10.11 | -7.19 | -4.7 | **NO-OP** (bit-identical vs rollwin on synth) |
| 174 | `iter174_rollwin_chop_risk11.yaml` | 0 | 0 | -inf | -10.11 | -10.11 | -7.19 | -4.7 | **NO-OP** (bit-identical vs rollwin on synth) |

---

## Wave P (175–178) — synthetic stand-in harness

| Iter | Config | cap | W | WS | minMA | Mar | Apr | full% | Verdict |
|------|--------|:---:|:--:|-----:|------:|----:|----:|------:|---------|
| 175 | `iter175_rollwin_chop_tp8.yaml` | 0 | 0 | -inf | -10.11 | -10.11 | -7.19 | -4.7 | **NO-OP** (bit-identical vs rollwin on synth) |
| 176 | `iter176_rollwin_trend_max3.yaml` | 0 | 0 | -inf | -10.11 | -10.11 | -7.19 | -4.7 | **NO-OP** (bit-identical vs rollwin on synth) |
| 177 | `iter177_rollwin_risk825.yaml` | 0 | 0 | -inf | -10.11 | -10.11 | -7.35 | -3.6 | **Revalidate** (synth: full% vs rollwin; gold) |
| 178 | `iter178_rollwin_chop_sl029.yaml` | 0 | 0 | -inf | -10.28 | -10.28 | -7.10 | -4.9 | FALSIFIED (synth; Mar worse) |

---

## Wave Q (179–182) — `zigzag_fib_mtf` — **GOLD** harness

`data/xauusd_m1_2026.csv` (108,871 M1 bars, Dukascopy Jan–Apr 2026),
`scripts/iter32_compare_configs.py` vs `adaptive_dual_pivot_chop_moon_r8_tp9_rollwin.yaml`
(default 4-window research / validation / test battery).

| Iter | Config | cap | W | WS | minMA | Mar | Apr | full% | Verdict |
|------|--------|:---:|:--:|-----:|------:|----:|----:|------:|---------|
| rollwin | (baseline) | 0 | 2 | 0.101 | 0.83 | 0.83 | 1.28 | 232.7 | — |
| 179 | `iter179_wave_q_zigzag_fib_baseline.yaml` | **1** | 0 | -inf | -18.79 | -1.36 | -18.79 | -41.3 | FALSIFIED |
| 180 | `iter180_wave_q_zigzag_thresh10.yaml` | **2** | 0 | -inf | -14.71 | -4.04 | -14.71 | -49.5 | FALSIFIED |
| 181 | `iter181_wave_q_zigzag_fibwide.yaml` | **1** | 0 | -inf | -16.16 | 8.85 | -16.16 | -38.1 | FALSIFIED |
| 182 | `iter182_wave_q_zigzag_thresh18.yaml` | 0 | 0 | -inf | -13.85 | -2.23 | -13.85 | -7.2 | FALSIFIED |

**Per-window (iter179, gold):** all four windows **failed** generalization (PF or sign mismatch): W1 val −5.9% / test −10.2%; W2 −16.8% / −14.4%; W3 −17.5% / +31.0% (sign flip); W4 +31.0% / −32.5% (sign flip).

---

## Wave R (183–186) — `zigzag_fib_mtf` tuning — **GOLD** harness

Same CSV and harness as Wave Q.

| Iter | Config | cap | W | WS | minMA | Mar | Apr | full% | Verdict |
|------|--------|:---:|:--:|-----:|------:|----:|----:|------:|---------|
| rollwin | (baseline) | 0 | 2 | 0.101 | 0.83 | 0.83 | 1.28 | 232.7 | — |
| 183 | `iter183_wave_r_zigzag_risk1.yaml` | 0 | 0 | -inf | -14.83 | -14.83 | -9.43 | -34.4 | FALSIFIED |
| 184 | `iter184_wave_r_zigzag_lonny.yaml` | 0 | 1 | 3.766 | -13.24 | 2.25 | -13.24 | -14.9 | FALSIFIED |
| 185 | `iter185_wave_r_zigzag_fibtight.yaml` | **1** | 0 | -inf | -10.78 | 15.31 | -10.78 | -36.2 | FALSIFIED |
| 186 | `iter186_wave_r_zigzag_nom1bias.yaml` | **7** | 0 | -inf | -22.63 | -22.63 | 2.32 | -38.0 | FALSIFIED |

---

## Wave S (187–190) — `zigzag_fib_mtf` tuning — **GOLD** harness

| Iter | Config | cap | W | WS | minMA | Mar | Apr | full% | Verdict |
|------|--------|:---:|:--:|-----:|------:|----:|----:|------:|---------|
| rollwin | (baseline) | 0 | 2 | 0.101 | 0.83 | 0.83 | 1.28 | 232.7 | — |
| 187 | `iter187_wave_s_zigzag_sl022.yaml` | **1** | 0 | -inf | -9.39 | -2.05 | -9.39 | -40.4 | FALSIFIED |
| 188 | `iter188_wave_s_zigzag_tpfast.yaml` | **1** | 0 | -inf | -3.97 | -3.97 | 0.16 | -33.4 | FALSIFIED |
| 189 | `iter189_wave_s_zigzag_thresh135.yaml` | 0 | 0 | -inf | -21.95 | 23.36 | -21.95 | -45.3 | FALSIFIED |
| 190 | `iter190_wave_s_zigzag_atr10.yaml` | **1** | 0 | -inf | -17.44 | -4.94 | -17.44 | -45.3 | FALSIFIED |

---

## Wave T (191–194) — `zigzag_fib_mtf` tuning — **GOLD** harness

| Iter | Config | cap | W | WS | minMA | Mar | Apr | full% | Verdict |
|------|--------|:---:|:--:|-----:|------:|----:|----:|------:|---------|
| rollwin | (baseline) | 0 | 2 | 0.101 | 0.83 | 0.83 | 1.28 | 232.7 | — |
| 191 | `iter191_wave_t_zigzag_leg065.yaml` | **1** | 0 | -inf | -17.66 | -3.24 | -17.66 | -40.9 | FALSIFIED |
| 192 | `iter192_wave_t_zigzag_cool14.yaml` | **1** | 0 | -inf | -22.47 | 2.85 | -22.47 | -40.7 | FALSIFIED |
| 193 | `iter193_wave_t_zigzag_fib038.yaml` | 0 | 0 | -inf | -7.64 | 9.83 | -7.64 | -37.7 | FALSIFIED |
| 194 | `iter194_wave_t_zigzag_tp30.yaml` | **1** | 0 | -inf | -16.51 | -4.25 | -16.51 | -40.9 | FALSIFIED |

---

## Wave U (195–198) — `zigzag_fib_mtf` combos — **GOLD** harness

| Iter | Config | cap | W | WS | minMA | Mar | Apr | full% | Verdict |
|------|--------|:---:|:--:|-----:|------:|----:|----:|------:|---------|
| rollwin | (baseline) | 0 | 2 | 0.101 | 0.83 | 0.83 | 1.28 | 232.7 | — |
| 195 | `iter195_wave_u_zigzag_fib038_lonny.yaml` | 0 | **3** | 2.229 | -14.67 | -1.33 | -14.67 | -27.7 | **Trade-off** (3/4, cap=0; WS & months vs rollwin) |
| 196 | `iter196_wave_u_zigzag_fib038_r1.yaml` | 0 | 0 | -inf | -10.80 | -10.80 | -10.11 | -32.0 | FALSIFIED |
| 197 | `iter197_wave_u_zigzag_thresh135_fib038.yaml` | **1** | 0 | -inf | -15.15 | 23.31 | -15.15 | -39.2 | FALSIFIED |
| 198 | `iter198_wave_u_zigzag_sl028_fib038.yaml` | **1** | 0 | -inf | -9.15 | 4.50 | -9.15 | -40.3 | FALSIFIED |

---

## Wave V (199–200) — `zigzag_fib_mtf` — **GOLD** harness (serial **103–200** closure)

| Iter | Config | cap | W | WS | minMA | Mar | Apr | full% | Verdict |
|------|--------|:---:|:--:|-----:|------:|----:|----:|------:|---------|
| rollwin | (baseline) | 0 | 2 | 0.101 | 0.83 | 0.83 | 1.28 | 232.7 | — |
| 199 | `iter199_wave_v_zigzag_tp085_22.yaml` | **1** | 0 | -inf | -5.15 | 2.47 | -5.15 | -39.7 | FALSIFIED |
| 200 | `iter200_wave_v_zigzag_thresh125_fib040.yaml` | **1** | 0 | -inf | -10.47 | 5.96 | -10.47 | -37.8 | FALSIFIED |

**Serial 103–200:** journal rows complete through **iter200** on gold for this `zigzag_fib_mtf` tranche.

---

## Wave W (201–204) — `zigzag_fib_mtf` refinements of iter195 — **GOLD** harness

| Iter | Config | cap | W | WS | minMA | Mar | Apr | full% | Verdict |
|------|--------|:---:|:--:|-----:|------:|----:|----:|------:|---------|
| rollwin | (baseline) | 0 | 2 | 0.101 | 0.83 | 0.83 | 1.28 | 232.7 | — |
| 201 | `iter201_wave_w_zigzag_fib040_lonny.yaml` | 0 | 0 | -inf | -9.70 | -6.92 | -9.70 | -28.9 | FALSIFIED |
| 202 | `iter202_wave_w_zigzag195_tp22.yaml` | 0 | **3** | **0.809** | -13.72 | -4.51 | -13.72 | -28.3 | **Trade-off** (3/4, cap=0; **WS beats iter195** ~0.81 vs ~2.23) |
| 203 | `iter203_wave_w_zigzag195_thresh115.yaml` | 0 | 0 | -inf | -11.85 | -4.66 | -11.85 | -42.4 | FALSIFIED |
| 204 | `iter204_wave_w_zigzag_fib038_overlap.yaml` | 0 | 1 | 2.357 | -3.05 | 6.95 | -3.05 | -3.9 | FALSIFIED (1/4; small full loss) |

**Next:** continue **205+** in this file or split a new journal per project preference.
