...
| `iter9/pivot_bounce` | iter10: daily floor-trader pivot S1/S2/R1/R2 reversal | Full +8.95% (¥+8,946), Validation +1.74% (PF 1.46), Research +22.59% (PF 1.79), Tournament 14d -6.41% | iter10 standalone winner (April hostile; positive on full+val+research, 0 cap viol) |
| `iter11/pivot_bounce_v2` | iter11: pivot_bounce with tighter touch_atr_buf=0.05 + sl_atr_buf=0.20 | Full +27.21% (¥+27,210), Research +39.97% (PF 2.32), Validation +4.23% (PF 2.18), Tournament 14d -9.38% / 7d -10.10% | iter11 standalone — superseded by iter13_v3 |
| `iter13/pivot_bounce_london_v3` | iter13: London-only pivot_bounce, TP1=1.0R, TP2=1.5R | Full +30.10%, Val +5.61% (PF 4.69), DD -10.7%, min_eq 97.6% | iter13 standalone — superseded by iter14 v4 |
| `iter14/pivot_bounce_london_v4` | iter14: v3 + risk pushed to 5.0% | Full +53.90%, Val +9.49% (PF 2.79), DD -20.6%, min_eq 95.3% | iter14 — superseded by iter15 v7 |
| `iter15/pivot_bounce_london_v7` | iter15: v4 + light DD throttle | Full +57.95%, Val +9.49% (PF 2.79), DD -18.5% | iter15 — superseded by iter17 v8 |
| `iter17/pivot_bounce_london_v8` | iter17: v7 + cooldown 60 + daily_kill 3 | Full +60.56%, Val +9.49% (PF 2.79), DD -15.5% | iter17 standalone — superseded by iter18 dual |
| `iter18/ensemble_pivot_dual` | iter18: pivot daily + weekly | Full +64.68%, Val +11.79% (PF 2.11), DD -19.2% | iter18 — superseded by iter19 triple |
| `iter19/ensemble_pivot_triple` | iter19: pivot daily+weekly+monthly | Full +69.17%, Val +15.20% (PF 2.50), DD -21.1% | iter19 — superseded by iter20 v2 |
| `iter20/ensemble_pivot_triple_v2` | iter20: triple at risk=7.0 | Full +102.43%, Val +17.65% (PF 2.15), DD -26.2% | iter20 BALANCED — superseded headlines on growth |
| `iter21/triple_aggressive` | iter21: triple at risk=10.0 | Full +108.63%, Val +15.04% (PF 1.69), DD -33.2% | iter21 AGGRESSIVE — superseded by iter22 v2 |
| `iter22/triple_aggressive_v2` | iter22: triple + daily SL 0.30 | Full +114.38%, Val +16.89% (PF 1.78), DD -33.2% | iter22 — superseded by iter23 v3 |
| `iter23/triple_aggressive_v3` | iter23: v2 + weekly slb 0.28 | Full +146.61%, Val +16.85% (PF 1.76), DD -31.8% | iter23 — superseded by iter24 v4 |
| **`iter24/triple_aggressive_v4`** | **iter24: v3 + weekly tp2_rr 2.0→2.5 (TWO-LEG TP1+TP2)** | **Full +166.23% (¥+266,231), Val +17.76% (PF 1.78), Tourn 14d -22.15%, DD -32.7%** | **PROJECT GROWTH RECORD — 2.66x; Jan +49.81, Feb +71.72** |
| `iter25/triple_single_tp` | iter25: single TP at 1.0R | Full +102.26%, Val +18.47% (PF 1.90), Tourn 14d -16.73%, DD -33.5% | iter25 — superseded by iter26 v2 |
| **`iter26/triple_single_tp_v2`** | **iter26: single_tp + risk=7 + concurrency=2** | **Full +103.17% (¥+103,175; ¥100k → ¥203,175), Validation +19.09% (PF 2.27), Tournament 14d -14.02% / 7d -8.20%, DD -27.8%** | **iter26 BEST validation PF on single-TP family + BEST tournament of any pivot variant; concurrency=2 lets daily+weekly fire together at lower per-trade risk** |
| **`iter27/v4_plus_sweep_r4`** | **iter27: v4 triple pivot + session_sweep_reclaim London at risk=4** | **Full +39.18% (¥+39,175), Validation +66.76% (PF 4.57!) — project-record validation PF, Tournament 14d -16.09%, DD -34.2%, 0 cap viol** | **iter27 PROJECT VALIDATION RECORD — sweep_reclaim adds huge val edge (Apr +49.21% in val window); Jan -13% drags full but every other month positive; ships as "best validation" headline alongside v4 growth and v2 balance** |
| `iter25/turn_of_month` | iter25: trade last 3 + first 2 days of month with HTF EMA bias | Full -14.82%, val negative | iter25 FALSIFIED |
| `iter25/asian_break_continuation` | iter25: opposite of sweep_reclaim — clean break + continuation | Full -0.81%, swings (Jan +42, Apr -30) | iter25 FALSIFIED standalone (high variance) |
| `iter25/atr_squeeze_breakout` | iter25: HTF ATR percentile <25% → 20-bar range break | Full -75% RUIN flag | iter25 FALSIFIED |
| `iter16/ensemble_london_v2` | iter16: pivot_london_v7 + sweep_reclaim London | Full +14.45%, Validation +17.33% (PF 3.05), Tournament 14d -11.99% | iter16 — strongest VAL PF in project (3.05) but sweep dilutes pivot's full-period |
| `iter12/keltner_mean_reversion` | iter12: Keltner channel reversal with EMA-slope gate | Full -0.31% baseline; 12-pt sweep: best in-sample +42.5% but ALL configs val PF < 1.0 | iter12 FALSIFIED (validation discipline catches in-sample overfit) |
| `iter12/order_block_retest` | iter12: ICT Order Block retest after Break of Structure | Full -24.3% baseline; 12-pt sweep: clean configs all val-negative; positive-val configs all trip cap_viol | iter12 FALSIFIED (val-clean = no edge; val+ = cap violations) |
