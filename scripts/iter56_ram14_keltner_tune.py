#!/usr/bin/env python3
"""Tune ram14 high-Mar/Apr stack: sweep Keltner risk_mult × kelt_mult.

Base: adaptive_triple_keltner_overlap_adx1424_kr046_km20.yaml
Phase 1: full-period only (fast). Phase 2: harness on top-K by min(Mar,Apr).

Example::

    python3 scripts/iter56_ram14_keltner_tune.py --csv data/xauusd_m1_2026.csv
"""
...
def main() -> None:
    ...
    ap.add_argument(
        "--base",
        default="config/research_aspiration_200/adaptive_triple_keltner_overlap_adx1424_kr046_km20.yaml",
    )
    ...
    krs = [0.36, 0.38, 0.40, 0.42, 0.44, 0.46]
    kms = [1.85, 1.88, 1.9, 1.95, 2.0]
    ...
    ok = [row for row, _ in screened if row.get("windows_passing") is not None]
        ...
        json.dumps(
            {
                ...
                "best_4of4_by_worst_score_then_min_mar_apr": ok_44[:15],
            },
            indent=2,
        )
    )
